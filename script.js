...
// it show's all meals  acording to search input value
    ...
    meals.then(data=>{
        if (data.meals) {
            data.meals.forEach((element) => {
                ...
                if (isFav) {
                    html += `
                <div id="card" class="card mb-4" style="width: 17rem;">
                    <img src="${element.strMealThumb}" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h5 class="card-title">${element.strMeal}</h5>
                        <div class="d-flex justify-content-between mt-5">
                            <button type="button" class="btn btn-outline-light" onclick="showMealDetails(${element.idMeal})">Show More Details</button>
                            <button id="main${element.idMeal}" class="btn btn-outline-light active" onclick="addRemoveToFavList(${element.idMeal})" style="border-radius:50% ho"><i class="fa-solid fa-heart"></i></button>
                        </div>
                    </div>
                </div>
                `;
        ...
        for (let i = 0; i < arr.length; i++) {
            await fetchMealsFromApi(url,arr[i]).then(data=>{
                ...
                <div id="card" class="card mb-4" style="width: 20rem;">
                    ...
                    <div class="card-body">
                        <h5 class="card-title" style="text-align:center">${data.meals[0].strMeal}</h5>
                        <div class="d-flex justify-content-between mt-5">
